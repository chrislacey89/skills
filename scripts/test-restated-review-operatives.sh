...
# the canon rather than restating it, and each carrying a planted self-test —
# a positive that must hit and a near-miss that must stay silent — because a
...
section "No restated operative survives in the census population"
...
assert_eq "" "$numeric_hits" "no dimension is referred to by number in any of $pop_count tracked files"
assert_eq "" "$gate_hits" "no dimension's '**Only runs when …**' gate is restated outside the canon ($gate_count gates extracted)"
assert_eq "" "$band_hits" "no file outside the canon restates the review-size ladder ($band_count thresholds extracted; Observation bound $observation_bound is canon-only)"
assert_eq "" "$label_hits" "every parenthetical severity label classifies into a tier the canon defines"
