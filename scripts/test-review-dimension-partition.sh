...
#
# The census that mattered here ran wider than /pre-merge. SYSTEM-OVERVIEW.md
# also stated the count — and enumerated every dimension by name — and it is
# bundled into four skills' references/ directories, so one uncaught site was
# really five. Its bundled copies are not scanned separately: they are
# generated by scripts/sync-skill-references.sh and pinned by the
# check-skill-references CI job, so the canonical file is the only place the
# claim can be authored. That is the whole reason to scan the source rather
# than the copies.
sysoverview="$repo_root/SYSTEM-OVERVIEW.md"
[[ -f "$sysoverview" ]] || fatal "SYSTEM-OVERVIEW.md not found at $sysoverview"

count_literals="$(grep -nE '\b(11|[Ee]leven)[- ](dimension|architectural review dimension)' "$premerge_skill" "$checklist" "$sysoverview" || true)"
assert_eq "" "$count_literals" "no literal dimension count survives in pre-merge/SKILL.md, review-checklist.md, or SYSTEM-OVERVIEW.md"
