...
```
/shape → /research (always, depth-calibrated) → /write-a-prd → /prd-to-issues → /execute → QA → /pre-merge → /compound (in-PR, when a lesson exists) → /pre-merge (re-run: the compound commit is the post-stamp delta; re-stamps) → /closeout (merge + teardown) → cleanup
```
...
It is one short block — not a new section, not a paragraph, not a copy-paste cheat sheet — and it draws its data from the skill's existing `## Handoff` section. `/compound` ends the loop only on its post-merge fallback path, where it prints `**Loop closed.** Next: /help when you return to this repo` instead; on its in-PR default path two steps still follow — a `/pre-merge` re-run reads the commit it just made and re-stamps, then `/closeout` merges — so `/compound` prints a normal `**Next session:** /pre-merge` block.
...
**Where it runs:** By default, **in-PR before merge** — after `/pre-merge` creates the PR and before `/closeout` merges it. `/compound` commits the `docs/solutions/` entry onto the open PR branch and hands back to `/pre-merge`, so the lesson is reviewed before the same merge and merged atomically with the code. The post-merge position implied by this step's number is the *fallback*, for lessons that only surface during or after the merge (integration surprises, QA findings). The step number reflects ownership of the tail, not the default runtime order — which is `/pre-merge → /compound (in-PR) → /pre-merge (re-run) → /closeout`. The return leg exists because committing the entry moves the head past the review-currency stamp and `/pre-merge` is that stamp's only writer: without it the entry is merged with the code that taught it and read by nobody (#336). The re-run does not re-recommend `/compound`, so the leg runs once — read off the delta when that commit is docs-only, and otherwise by `/pre-merge` Phase 4's own *"if — and only if — a durable lesson emerged"* condition, because the lesson is already captured in the commit under review (#367).
...
| `/pre-merge` | Verified implementation ready to review (author-mode), an existing PR number to review (reviewer-mode via `--pr <n>`), or an AFK handoff from `/execute` (loop-mode via `--loop`) | PR with lineage, stamped with the reviewed head SHA, plus architectural review readout (author-mode); draft PR comment text shaped by `references/comment-craft.md` (reviewer-mode); or that same PR plus a `## Review Disposition Ledger` giving every finding a row, an owner, and its supporting evidence (loop-mode) | `/compound` in-PR when a lesson emerged — which hands back here for a re-run over its commit — then `/closeout` to merge and tear down; loop-mode first hands the ledger's `open` rows back to the operator to settle |
| `/closeout` | Reviewed, mergeable PR from `/pre-merge` (its lesson, if any, already compounded onto it in-PR and re-stamped by the `/pre-merge` re-run that followed) on a feature branch (worktree or plain checkout) | Merged PR, torn-down worktree, pruned branch, pulled base, verified clean end state | Step 9 issue-closing; post-merge `/compound` only as the fallback for a lesson that surfaced at/after merge |
| `/compound` | A durable lesson — known at PR time (in-PR default) or surfacing at/after merge (post-merge fallback) | `docs/solutions/` entry with volatility and Shelf Life, riding the PR (in-PR) or committed to base (post-merge) | In-PR: a `/pre-merge` re-run in author-mode takes the compound commit as its post-stamp delta and re-stamps, then `/closeout` merges the PR carrying it. Post-merge: loop closed. Future `/research` and `/write-a-prd` consult it |
...
| `/fix-findings` | The `/pre-merge` finding numbers a human chose, plus the findings themselves (terminal output, or the PR ledger loop-mode wrote) | One commit per accepted finding, each authored by a fresh sub-agent that did not write the code; a per-finding breaker verdict (`killed` / `survived` / `not-run` / `not-applicable`), the SHA it archived, and the command behind it; `refuted` reports for findings the tree did not confirm. No stamp, no PR edit, no merge | a re-run of `/pre-merge` in author-mode, which reads the stamp off the PR, takes the post-stamp delta — these fix commits — as its subject with the tree readable around it, and re-stamps at the new head (whole branch again in the four states `pre-merge/SKILL.md` Phase 1 step 4 names); then `/compound` if a lesson emerged — whose own commit sends a second re-run through the same mechanism — then `/closeout` |
...
- `/init-pipeline` is auto-invoked by `/execute` Step 0 when `.claude/hooks/enforce-classification.sh` is missing, or when it exists but does not read `.claude/.review-stamped` (a pre-lock install, re-scaffolded without re-asking the trigger-surface question) — scaffolds Claude Code hooks (TDD classification gate, git guardrails), pre-commit hooks, and package manager enforcement
...
- `/pre-merge` → `/compound` (in-PR, when a durable lesson exists — captured onto the open PR) → `/pre-merge` (re-run: the compound commit is the post-stamp delta, so the entry is reviewed and the stamp moves to the head that merges) → `/closeout` (merge + worktree teardown); post-merge `/compound` remains the fallback for lessons that surface at/after merge
- `/pre-merge` Phase 4's next-step menu → `/fix-findings` → a `/pre-merge` re-run scoped to the post-stamp delta → `/closeout`. This is the branch the human takes when a finding should be fixed on this branch rather than handed off as an issue. `/fix-findings` is user-invoked only and never self-triggers: it takes the numbers the human chose, spawns one fresh fixer per finding under `/pre-merge` Phase 3's context contract, spawns a fresh read-only breaker in a `git archive` copy that validates its apparatus before reporting a verdict, and caps at one fixer pass and one breaker pass per finding. It never selects findings (#190's boundary), never merges, and never stamps — `/pre-merge` remains the stamp's only writer, pinned by `scripts/test-review-currency-marker.sh`. Backing it is the **post-review edit lock**: `/pre-merge` Phase 4 writes `.claude/.review-stamped` beside the stamp, the classification hook refuses implementation writes while that flag exists and `.claude/.fix-findings-active` does not, and `/closeout` removes it at merge. The refusal names two routes — invoke the skill, or delete the flag by hand — which makes visible a choice that is silent today. It is a stop on the default post-review edit rather than an enclosure: the hook is registered on `Write|Edit` and refuses only implementation-pattern paths, so a `Bash` write, a write to either flag, and a write to the hook or `.claude/settings.json` all go through untouched — `scripts/test-post-review-edit-lock.sh` § 4 runs each one and records what it lets through
