...
**Where it runs:** By default, **in-PR before merge** — after `/pre-merge` creates the PR and before `/closeout` merges it. `/compound` commits the `docs/solutions/` entry onto the open PR branch and hands back to `/pre-merge`, so the lesson is reviewed before the same merge and merged atomically with the code. The post-merge position implied by this step's number is the *fallback*, for lessons that only surface during or after the merge (integration surprises, QA findings). The step number reflects ownership of the tail, not the default runtime order — which is `/pre-merge → /compound (in-PR) → /pre-merge (re-run) → /closeout`. The return leg exists because committing the entry moves the head past the review-currency stamp and `/pre-merge` is that stamp's only writer: without it the entry is merged with the code that taught it and read by nobody (#336). The re-run does not re-recommend `/compound`, so the leg runs once.
