...
**What it does:** Creates a GitHub PR with full lineage (links PRD issue, lists closed/open slice issues) and runs an architectural review of the full diff against project principles. Checks eleven dimensions, defined in `pre-merge/review-checklist.md`: deep modules, vertical slice integrity, state discipline, boundary map contracts, coverage matrix reconciliation, test quality, docs/solutions/ adherence, runtime initialization and production-runtime parity, fix completeness, surgical scope, and review-friendly size. Four gate themselves off when the diff gives them nothing to check — boundary maps and coverage matrix need a PRD with slice issues, runtime initialization needs schema/config/CLI/deploy-runtime changes, and fix completeness needs a bug fix (it catches workarounds disguised as corrections, missing regression tests, and lone instance fixes). Findings are advisory — presented in the terminal as Observations, Suggestions, or Concerns. When a PRD with slice issues was provided, also notes scope drift: work that appeared in the diff but wasn't in any slice's Boundary Map, and declared Produces that weren't implemented.
