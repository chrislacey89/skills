...
**Three modes:** *author-mode* (default — creates the PR, prints advisory findings; auto-invoked by `/execute` Step 6 on the HITL path), *reviewer-mode* (`--pr <n>` — reviews someone else's PR and drafts comment text), and *loop-mode* (`--loop`, or auto-entered when `/execute` hands off on an AFK run).

**Loop-mode (the third missing loop):** review was a sensor with no acknowledgement channel. `/pre-merge` emitted findings into a terminal and closed with "No action is required" — nothing tracked whether a finding was acted on, filed, or dropped, so the only thing closing the loop was the operator's memory, and every AFK night produced a branch nobody had reviewed. Loop-mode gives every finding a disposition: **fix it here** (local to the diff and confirmable by a named feedback loop) or **escalate and stop** (public interfaces, exported types, boundary-map `Produces`, API contracts, dependency changes, or a fix whose minimal and thorough forms leave different contracts behind). Filing is the output of the structural-sibling search a fix already runs, not a third branch. It plans all dispositions before executing any, verifies each finding against the tree before acting on it, applies fixes through `/execute`'s normal path, re-stamps, and re-reviews only the delta — bounded at three passes, with a `## Review Disposition Ledger` in the PR body carrying dispositions forward while withholding prior severity judgments so re-raised findings stay real signal. **Its exit condition is that every finding has a disposition — never that the review came back clean**, which would be the same "zero findings" signal Phase 4's minimum-findings guard already distrusts (Meadows, *Seeking the Wrong Goal*; Leveson, "reward reporting, never low counts"). Hitting the three-pass bound with findings still undispositioned is an escalation, not a clean exit. The re-stamp is conditional and that is the point: the loop's fix commits are invalidating writers in the interval `/closeout` checks, self-remediating only on the clean path where a delta pass actually read them. On the escalation and bound-exhaustion paths the stamp is deliberately left behind the head, because those fixes were applied and never reviewed — `/closeout` surfacing that delta is a true positive, and re-stamping would suppress a real signal by asserting a review that never ran.
...
| `/visual-recap` | Finished diff, PR, or branch to comprehend, plus the base branch to diff against | Transient self-contained interactive HTML recap (file-tree + change flags, annotated split diffs with line-anchored callouts, schema/API contract cards, UI wireframes, before/after columns, CSS-first diagrams with Mermaid opt-in) and the reviewer's copied-back feedback, with durable items promoted to GitHub PR review comments | `/closeout` to merge and tear down, then `/compound` only when a durable lesson emerged |
...
- `/visual-recap` is an optional side-route at the same `/pre-merge` → merge boundary: it renders a finished diff/PR/branch as a transient self-contained interactive HTML recap (line-anchored callouts, before/after, reused `/mermaid`) so a reviewer who didn't author the change can grasp its shape before reading lines. `/pre-merge` Phase 4 may recommend it like `/walk-commits`; it is never auto-invoked, skips small/obvious diffs, finds no defects (that stays `/pre-merge`), and the HTML is transient (gitignored `.context/` or `mktemp`, never committed). It shares the `docs/visual-rendering-core.md` rendering core (vocabulary, copy-text feedback serializer, Grounding Rule, secret-redaction, Tufte quality bar) with `/walk-commits`
...
| Drive review to done without sitting through each pass | `/pre-merge --loop` — loop-mode; bounded at three passes, gives every finding a disposition (fix here or escalate), auto-runs `/ts-audit` on changed `.ts`/`.tsx`, and is what an AFK `/execute` hands off to |
