...
4. **Make an illegal input unconstructable, not merely detectable**
   ...
   > "Validating after construction is strictly weaker than making invalid construction impossible, because a runtime check only protects the one call site it guards."
   ...
   **The TypeScript translation.** Wlaschin's smart constructor is a branded type plus a constructor that is the only way to make one:
   ...
   function containsVerbatim(needle: string, haystack: string): boolean {}
   ...
   function containsVerbatim(needle: string, haystack: ReadingText): boolean {}
   ```
   ...
   **Proportion.** This is not "brand every primitive." Wlaschin calls maximal application "probably design overkill" for his own domain and frames the guideline as proportionate use of one trick. Reach for it when the constraint is one a *downstream* function depends on and cannot re-derive — provenance, validation status, units, cardinality. A value that never crosses a function boundary does not need a brand, and a rule that reads as "wrap every string" will be ignored or, worse, followed.

   **Changing an existing type is a separate move** with its own discipline — see [refactoring.md](refactoring.md) § Discipline, "Change the type first."
