...
- [ ] **Consolidate a restated claim.** When a claim, standard, or contract is stated at more than one operative site in a prose artifact the toolchain does not check — an LLM prompt, a policy string, an error-message contract — extract it to a single interpolated constant rather than editing each site. Assert the literal appears once. A prose contract has no negative space: an additive edit leaves the superseded statement co-resident with the new one, and both satisfy a substring search, so a guard test written against the new wording passes while the old wording is still the one some readers reach. Cite: Ousterhout, information leakage — "a design decision reflected in multiple modules."
