...
- [ ] **Consolidate a restated claim.** When a claim, standard, or contract is stated at more than one operative site in a prose artifact the toolchain does not check — an LLM prompt, a policy string, an error-message contract — extract it to a single interpolated constant rather than editing each site. **Census before you consolidate, and enumerate by what the claim does** — every verdict, every scored criterion, every field a caller branches on — not by grepping the phrasing you already found; the site that escapes is the one worded differently or living in a different file class. A prose contract has no negative space: an additive edit leaves the superseded statement co-resident with the new one, and both satisfy a substring search, so a guard test written against the new wording passes while the old wording is still live. Asserting the literal appears once pins that nobody re-inlined the constant — it cannot see a restatement in fresh words, so the census, not the assertion, is what has to be complete. When the artifact cannot interpolate (text that must ship inline across independently-installed copies), reference the canonical statement by name or section instead of restating part of it. Cite: Ousterhout on information leakage, "a design decision reflected in multiple modules"; Kuhn's CNL survey on Caterpillar Fundamental English, discontinued in 1982 because its guidelines "were not enforceable in the English documents produced" (p. 15).
