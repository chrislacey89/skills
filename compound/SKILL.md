...
**The re-run terminates here; it does not bounce back.** Its subject is the entry that captured the lesson, so no *uncaptured* lesson can emerge from reading it. What carries that depends on what this commit contains, and both cases are `/pre-merge` Phase 4's own — neither is a discriminator this file adds. **Docs-only:** Phase 4's path test reads the post-stamp delta, finds nothing outside `docs/solutions/`, and suppresses the recommendation mechanically. **Carrying Phase 4's mechanism as well** — which the `git add` above stages, so expect it on roughly half of captures by the measurement in the paragraph before this one: the delta holds paths outside `docs/solutions/`, the path test is deliberately scoped not to fire there (`/pre-merge` says so where it defines the test), and what governs instead is Phase 4's own *"if — and only if — a durable lesson emerged"* condition, because a lesson captured in the commit under review has not emerged uncaptured. So on that path the guarantee is the judgment clause's, not the test's — read as a promise the mechanism keeps unconditionally, it is broader than the mechanism. There is one re-run per in-PR capture either way, and its exit is `/closeout`.
