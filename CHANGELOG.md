...
- **Reads that need a per-edge `state` stay on `gh api`** — the inner node shape of `--json blockedBy` could not be verified against a live edge during implementation, and an unverified `jq` path does not belong in a skill. The dependency-list endpoint is the verified source for edge state; the summary counts have no `--json` equivalent at all and are absent from the single-issue `GET`, which is now documented as a sharp edge in its own right.
