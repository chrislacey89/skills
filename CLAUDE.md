...
- `/tdd` and `/pre-merge` share `docs/restated-claims.md` — the single statement of the restated-claim-in-unchecked-prose class (what a prose contract and an operative site are, the census move, the limit of the literal-count assertion, and the fallback when interpolation is unavailable). `/tdd` § 4 Refactor points at it for the author-time move; `/pre-merge` Dimension 1 points at it for the review-time detector. Neither restates it — bundled into both via `scripts/skill-references.manifest`, and `scripts/test-bundled-pointer-resolution.sh` pins that both pointers resolve
