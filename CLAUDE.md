...
- `/init-pipeline` is auto-invoked by `/execute` Step 0 when `.claude/hooks/enforce-classification.sh` is missing, or when it exists but does not read `.claude/.review-stamped` (a pre-lock install, re-scaffolded without re-asking the trigger-surface question) — scaffolds Claude Code hooks (TDD classification gate, git guardrails), pre-commit hooks, and package manager enforcement into the target project
...
- `/fix-findings` is the fix half of the review seam, and the pack's first **user-invoked** skill (`disable-model-invocation: true`, per § *Invocation mechanics* — it can never usefully self-trigger, because its only input is a set of finding numbers a human typed). It takes those numbers, spawns one fresh fixer sub-agent per finding under `/pre-merge` Phase 3's context contract, then one fresh read-only breaker in a `git archive` copy, and caps at one fixer pass and one breaker pass per finding. Three boundaries are load-bearing and must not erode in a later edit: **it never selects findings** (#190 — disposition is the human's; a finding the tree does not confirm is reported `refuted`, not decided away), **it never writes the review-currency stamp** (`/pre-merge` is the only writer, pinned by `scripts/test-review-currency-marker.sh`), and **it never merges**. It is backed by the post-review edit lock — `/pre-merge` Phase 4 writes `.claude/.review-stamped`, `/init-pipeline`'s classification hook refuses implementation writes while it exists and `.claude/.fix-findings-active` does not, and `/closeout` removes it at merge
...
- `/compound` captures lessons into `docs/solutions/` — by default onto the open PR before `/closeout` merges (so the lesson is reviewed and merged with the code that taught it), or post-merge as the fallback for lessons that surface at/after merge; this is the compounding loop, and it may also capture tranche-level lessons when a milestone closes. **The in-PR path's terminal step is a `/pre-merge` re-run, not `/closeout`** (#336): committing the entry onto the branch moves the head past the review-currency stamp, `/pre-merge` is that stamp's only writer, and without the hand-back the entry merges with the code that taught it and is read by nobody — which is the one thing the in-PR default was chosen to avoid. The re-run takes the compound commit as its post-stamp delta and re-stamps; it does not re-recommend `/compound`, so the leg runs once — read off the delta when that commit is docs-only, and otherwise by Phase 4's own *"if — and only if — a durable lesson emerged"* condition, which the path test is deliberately not widened to cover (#367). `/closeout` Step 2 and `/pre-merge` both judge such a commit by its paths rather than its `docs: compound` subject, because Phase 5 stages Phase 4's mechanism in the same commit — pinned by `scripts/test-compound-restamp-handoff.sh`
...
- `/execute` and `/fix-findings` share `docs/mutation-at-consumption.md` — the single statement of the mutation rung (name the property, mutate at the point of consumption, draw the edit from the corpus, and the two readings of a green result), plus the apparatus check that gates its four verdicts. `/execute` Step 4 points at it for the author-time move; `/fix-findings` points at it for the breaker's procedure. Neither restates it — bundled into both via `scripts/skill-references.manifest`
- `/tdd`, `/pre-merge`, `/execute`, and `/fix-findings` share `docs/restated-claims.md` — the single statement of the restated-claim-in-unchecked-prose class (what a prose contract and an operative site are, which kinds of text are not prose contracts, the census move, the limit of the literal-count assertion, and the fallback when interpolation is unavailable). `/tdd` § 4 Refactor points at it for the author-time move; `/pre-merge`'s Deep Modules dimension points at it for the review-time detector, and its Severity Classification points at the list of kinds that are not prose contracts to cap a finding on one at Suggestion; `/execute` Step 4 points at it for the scope check on set-claims, the author-path rung that keeps an existential measurement from being written up as a universal claim; `/fix-findings` Step 1 points at the same list to scope its closed action set for a finding about a sentence's claim — delete the sentence or point it at the canonical statement, never reword it, with the after-state read off the fix and a reworded fix reverted. None restates it — bundled into all four via `scripts/skill-references.manifest`; `scripts/test-bundled-pointer-resolution.sh` pins that the pointers resolve, and `scripts/test-prose-fix-disposition.sh` pins that the section they cite exists and that `/fix-findings`' after-state vocabulary matches its report block
- `/lfg` is a user-invoked side-route (`disable-model-invocation: true`) for a proof of concept. It auto-sizes the ask with `docs/trivial-task-rule.md` (bundled). A one-off fix takes a tiny path under the existing skip marker. Anything else takes a feature path: a short HITL shaping conversation, then AFK through research-lite, a PRD-lite issue, `/init-pipeline` declared on Path B, a tracer-bullet build loop capped on slices and attempts, and a close that removes `.claude/.lfg-active` before `/pre-merge --loop`. It reuses only `/init-pipeline` Path B and `/pre-merge --loop`, which reconnect it to the main workflow. It never writes the review-currency stamp, never fixes a finding in-run, never merges, never creates per-slice issues, and never advances a ledger row past `filed`. Its marker set is pinned by `scripts/test-lfg-marker.sh`
...
- **Invoked helper skills** — usually not top-level entry points for a feature, but delegated when a narrower decision is unresolved: `/api-design-review`, `/design-an-interface`, `/tdd`, `/triage-issue`, `/fix-findings`
- **Side-route skills** — valid alternate or supporting paths beside the main pipeline: `/qa`, `/request-refactor-plan`, `/improve-codebase-architecture`, `/improve-pipeline`, `/ubiquitous-language`, `/ts-audit`, `/walk-commits`, `/visual-recap`, `/help`, `/correct-course`, `/handoff`, `/re-pitch`, `/lfg`
