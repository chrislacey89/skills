...
- **Small diff** (< 200 changed lines, < 10 files): one sub-agent runs every dimension the checklist marks as sub-agent work and whose own gating condition is met (several gate themselves off — see the per-dimension notes below).
...
This is deliberately not a list. The list that used to sit here was a hand-maintained copy of the checklist's roster, and it shipped wrong: it never included Coverage Matrix Reconciliation, so on exactly the diffs large enough to split — which is to say the terminal PR of a multi-slice PRD, the only PR that dimension applies to — the one dimension carrying Blocker authority never ran, while Phase 5's ledger recorded that it had.

**Two separate repairs came out of that, and it is worth not confusing them.** Moving Coverage Matrix Reconciliation to Phase 4 is what makes it *run*; derivation would not have achieved that on its own, because reading its marker correctly still yields "do not delegate this." What derivation fixes is the *next* one: a dimension can no longer be added to the checklist without being assigned at the same moment it is defined, and `scripts/test-review-dimension-partition.sh` fails on any dimension left unassigned. The first repair fixes the instance; the second closes the class.
