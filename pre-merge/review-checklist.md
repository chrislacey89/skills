...
Used by `/pre-merge` during Phase 3 in all three modes — author-mode, reviewer-mode, and loop-mode. Each numbered dimension below is independent; the `## N.` headings **are** the roster, so nothing outside this file needs to restate how many there are. For every finding, classify as Observation, Suggestion, or Concern using the severity rules at the bottom.
...
- **Restated claim in unchecked prose** — the diff changes a claim inside a *prose contract*, and the same claim is stated at other *operative sites* — whether the diff leaves them untouched **or restates them in this same change**. Both halves matter: a diff that writes one claim into two files at once ships the divergence on arrival, and that is how this bullet's own predecessor shipped three. Both terms and the census procedure are defined in [references/restated-claims.md](references/restated-claims.md) § *The census comes before the remedy*; run it and cite each divergent or un-updated site by `file:line`. **Severity split:** **Concern** when a surviving site is one a caller acts on; **Suggestion** when the surviving sites are explanatory only. The fix is structural, not attentional — **do not write the remedy from this bullet**; send the author to the same reference. Do not recommend "check the other mentions too"; that is the attentional version, and it is what already failed. **Review-cadence note.** One downstream incident (#280) plus this repo's own record at `docs/solutions/architecture-decisions/by-construction-claims-need-a-mechanism-2026-08-11.md:184`. Same falsification rule as its siblings: if after a reasonable sample of PRs it fires <10% of the time on diffs that had no other reported issue, remove it rather than leave it as ceremony.
...
- Name-assertion divergence — the test name claims a *relationship* (a bound, an ordering, a rejection, a deadline) and the body asserts equality to a literal, so the assertion cannot fail in the direction the name claims. `test("max_tokens defaults below the non-streaming ceiling")` over `expect(client.maxTokens).toBe(16_000)` pins the literal and leaves the relational claim unfalsifiable — no run can violate it, and it stays true-looking the day the ceiling moves. Distinct from the implementation-coupled-names bullet above: that is WHAT-vs-HOW, this is name-vs-assertion, and a name can pass either and fail the other. Cite the name and the assertion. The fix is structural, not attentional — reference the bound or narrow the name — but **do not write the remedy from this bullet**: which of the two applies depends on whether the value under test is derived from the bound, and in that case neither does. Send the author to `tdd/tests.md` § *A test name is a claim the assertion must be able to falsify*, which carries the routing and the carve-out. Do not recommend "check the name matches the body"; that is the attentional version, and it is what already failed. **Severity split:** **Suggestion** by default; **Concern** when the named relationship is to a value the same diff introduces or one in a dependency, since the name is then the only record of a constraint nothing enforces. **Review-cadence note.** One downstream incident (2026-08-07, an LLM-client swap; fifth instance of the shape in that repo) plus Ammann & Offutt on requirement-vs-oracle. It caught 5/5 attentionally, so this makes a working detector mechanical rather than fixing a failure. Same falsification rule as its siblings: if after a reasonable sample of PRs it fires <10% of the time on diffs that had no other reported issue, remove it rather than leave it as ceremony.
- Fact-emitting heuristic with an untested failure direction — the diff adds or changes a module that reduces input to a boolean/category and whose output is consumed downstream as *trusted ground truth* (most sharply, a mechanical layer feeding an LLM judge in a hybrid mechanical-then-LLM dimension), and its tests cover only the happy direction. Confirm both directions are tested: an input that must **not** trigger the fact (over-claim guard) and one that **should** in a non-obvious form (under-claim guard). **Scope the over-claim guard per field-provenance, not per module** — a module-scoped reading passes the moment *any* guard exists, which is how a guard pinning the wrong field reads as coverage. Group the fields the heuristic reads by where the value comes from (extracted prose vs. raw markup, user-supplied vs. system-generated) and ask per group whether a guard pins *that* group; one proven against a single provenance is untested against another, not weakly tested. **Prefer the type-level fix to N guards** — make provenance part of the input's type (branded type, discriminated union, validating constructor at the boundary) so a second provenance cannot arrive silently. Wlaschin: validating after construction is strictly weaker than making invalid construction impossible, *because a runtime check only protects the one call site it guards*. Per-provenance guards are the fallback for when the type cannot change here. The tell is a **widened input domain with no new guard** — most visibly a parameter renamed from a provenance-specific name to a neutral one, which deletes the only place the precondition was recorded. **Severity asymmetry** (Nygard fault→error→failure — a wrong fact becomes an error the moment the consumer trusts it): an untested **over-claim** path is a **Concern**; an untested **under-claim** path is a **Suggestion**. Cite the specific untested input class (e.g. "a `User-agent`-scoped block read as block-all"), not "needs more tests." **Review-cadence note.** Added from one triggering incident (a downstream page-analysis repo's mechanical SEO layer — three failure-direction defects shipped green, 2026-06-23) plus principle grounds (Cohen phase-injection economics, Nygard fault→error→failure). The field-provenance scoping was added later from a second downstream incident (#266, 2026-08-22 — a verbatim-quote check widened from extracted page text to raw HTML; a module-level over-claim guard existed and pinned the old provenance, so this bullet as previously written never fired). Cadence applies to each half independently: if after a reasonable sample of PRs either fires <10% of the time on diffs that had no other reported issue, remove that half rather than leave it as ceremony.
...
- **Refactor-and-feature interleave** — Beck's "shame, shame": a behavior change and a structural cleanup mixed into the same diff, so no part of it can be taken as pure movement and every line has to be checked for behavior; where the repo preserves individual commits on base, the same mix inside one commit also leaves a future bisect unable to attribute regressions
...
**A comment is never the remedy.** A finding must not propose adding or expanding a code comment as its fix, under any dimension. Nothing acts on a comment (the same section), so a finding whose only remedy is one has named no fix: either the code can change so the reader no longer needs telling, or what they needed telling belongs in the PR body, or the finding is an Observation. This closes the loop #357 recorded, where "document the boundary in a comment" was offered as a remedy equal to fixing a production defect, and where authors learned to pre-empt review by arguing their case in comments. The reviewer names the code-shaped fix or the PR-body destination; the author decides between them.
