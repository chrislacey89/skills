...
- **Refactor-and-feature interleave** — Beck's "shame, shame": a behavior change and a structural cleanup in the same hunk, so "this part only moves code" stops certifying anything and the hunk has to be re-read line by line. Where the repo preserves individual commits on base, the same mix within one commit also leaves any future bisect unable to attribute regressions
