...
**Principle:** State lives in GitHub (issues) or in code (`docs/solutions/`, skills). No ad-hoc filesystem state. The enforcement state that decides whether work is acceptable — CI workflows, git-hook config, the contract tests they run, and any agent-side hook or classification marker — is state too, so *changing* it is in scope here, not only creating it.
...
- Enforcement artifact changed as a passenger — the diff touches an artifact that decides whether the work around it is acceptable, and that change is not the stated task (as Dimension 10 defines "stated task"; if none can be reconstructed, note that and skip this pattern rather than synthesizing one). **Identify the surface from the repo, not from a fixed list** — it is whatever this project's work is actually judged by, and it differs by stack. In a project `/init-pipeline` scaffolded: `.claude/hooks/**`, `.claude/settings.json`, and the `.claude/.tdd-active` / `.claude/.tdd-skipped` markers. In a repo whose gate is CI: the workflow definitions, the git-hook config (`lefthook.yml`, `.pre-commit-config.yaml`), and the contract tests wired into them. In this repo, that is `.github/workflows/`, `lefthook.yml`, `scripts/test-*.sh`, and this checklist itself. The process being judged writes all of them with ordinary tools, and the gates do not gate themselves: the scaffolded classification hook fires on an extension allowlist that its own `.sh` files fall outside, so an edit to the gate passes through the gate. **This check is detective, not preventive** — it reads the diff, so it sees the path whether the change was written by `Edit`, `sed -i`, or a `Bash` heredoc, and it makes the change visible rather than attempting to stop it. A classification marker appearing in the diff at all is a finding on its own: `/execute` Step 6 deletes both before handoff, so a committed marker means the gate's own bookkeeping leaked into the tree. When the change *is* the stated task — `/init-pipeline` scaffolding a project, a deliberate hook fix, a new contract test — that is not a violation, but say so explicitly in the findings rather than staying silent, so the reviewer can see the gate change was noticed and accepted. **The sharpest case in a CI-gated repo is not an added test but a relaxed one:** a diff that weakens an existing assertion — narrowing what a contract test covers, adding a skip, loosening a threshold — while also changing the thing that assertion covered. That is the self-issued `.tdd-skipped` in another costume, and unlike a new hook it looks like ordinary test maintenance. **Review-cadence note.** Added without a triggering incident, on principle grounds (Meadows Leverage Point #5: power over the rules outranks power under them; Strathern/Goodhart: an agent rationally responding to a reachable signal, no bad intent required) plus comparative source analysis of `coleam00/skills`' `build-dark-factory` (`.claude/skills/build-dark-factory`, read 2026-08-15), which auto-rejects any PR touching its three governance files. **Two retirement tests, and "fires" means a *violation*** — the acceptance notes this bullet mandates on stated-task edits are not fires, or the count could never fall. *Dead ceremony:* remove it if, over a reasonable sample, it produces a violation on <10% of diffs that had no other reported issue. *Noise:* remove it if over the same sample the acceptance notes outnumber the violations by more than roughly 5:1 — it is then taxing every PR to catch nothing, which #224 names as this bullet's top repo-wide risk. Neither count is instrumented: Phase 5's ledger is loop-mode only and its `Dimension` column is dimension-level, not per-bullet, so both are counted by hand by whoever next audits this file — the same honor system the sibling clauses run on, stated rather than implied.
