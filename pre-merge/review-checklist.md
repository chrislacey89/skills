...
- **Restated claim in unchecked prose** — the diff changes a claim inside a *prose contract* (text no compiler relates to its other statements; defined with its examples in [references/restated-claims.md](references/restated-claims.md)), and the same claim is stated at other operative sites the diff leaves untouched. The miss is silent: the superseded statement stays live beside the new one, and both satisfy any substring search the author used to check their own work. **Verification procedure — not eyeballing.** Read the whole artifact the diff edits, not the hunks, and enumerate the sites by what the claim does rather than by the phrasing the diff introduced. Cite each un-updated site by `file:line`. **Severity split:** **Concern** when a surviving site is one a caller acts on; **Suggestion** when the surviving sites are explanatory only. The fix is structural, not attentional — **do not write the remedy from this bullet.** Send the author to the same reference, which carries the census move and the fallback. Do not recommend "check the other mentions too"; that is the attentional version, and it is what already failed. **Review-cadence note.** One downstream incident (#280 — six operative sites in one critic prompt, five fixed, the fix reintroducing the class at the sixth) plus this repo's own record at `docs/solutions/architecture-decisions/by-construction-claims-need-a-mechanism-2026-08-11.md:184`: three instances of a prose rule failing to fire for a reader who had read it, against zero instances of it firing at authoring time. Same falsification rule as its siblings: if after a reasonable sample of PRs it fires <10% of the time on diffs that had no other reported issue, remove it rather than leave it as ceremony.
