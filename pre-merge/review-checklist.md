...
- **Restated claim in a prose artifact the toolchain does not check** — the diff edits a standard, verdict definition, scored criterion, or field contract inside an LLM prompt, policy string, or error-message template, and the same claim is stated at other sites the diff leaves untouched. This is information leakage with no compiler relating the sites, so the miss is silent: an additive edit leaves the superseded statement co-resident with the new one, and both satisfy any substring search the author used to check their own work. **Verification procedure — not eyeballing.** Read the *whole* artifact the diff edits, not the hunks; enumerate the sites by what the claim does (each verdict, each scored criterion, each field a caller branches on) rather than by the phrasing the diff introduced, and confirm every one carries the new standard. Cite each un-updated site by `file:line`. **Severity split:** **Concern** when a surviving site is one a caller branches on (the router reads a stale contract); **Suggestion** when the surviving sites are explanatory only. The fix is structural, not attentional — send the author to `/tdd` § 4 Refactor *Consolidate a restated claim*, which carries the census move and the case where interpolation is unavailable. Do not recommend "check the other mentions too"; that is the attentional version, and it is what already failed. **Review-cadence note.** One downstream incident (#280 — six operative sites in one critic prompt, five fixed, the fix reintroducing the class at the sixth) plus Kuhn's CNL survey on Caterpillar Fundamental English, discontinued in 1982 because its rules were prose rather than a checkable grammar (p. 15). The author-time bullet without this detector is that same shape: `by-construction-claims-need-a-mechanism-2026-08-11.md:184` records three instances of a prose rule failing to fire for a reader who had read it, against zero instances of it firing at authoring time. Same falsification rule as its siblings: if after a reasonable sample of PRs it fires <10% of the time on diffs that had no other reported issue, remove it rather than leave it as ceremony.
