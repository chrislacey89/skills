...
**Cross-references here name dimensions by title, never by number.** The `## N.` numbering is an artifact of reading order, and inserting a dimension renumbers everything below it, silently retargeting every reference written by number. Titles survive renumbering. `scripts/test-review-dimension-partition.sh` already pins its own dimension by title for the same reason, and `scripts/test-restated-review-operatives.sh` fails on any by-number reference in a tracked file.
