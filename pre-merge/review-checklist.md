...
- Name-assertion divergence — the test name claims a *relationship* (a bound, an ordering, a rejection, a deadline) and the body asserts equality to a literal, so the assertion cannot fail in the direction the name claims. `test("max_tokens defaults below the non-streaming ceiling")` over `expect(client.maxTokens).toBe(16_000)` is green forever and stays green the day the ceiling moves. This is distinct from the implementation-coupled-names bullet above: that one is WHAT-vs-HOW, this one is name-vs-assertion, and a name can pass either and fail the other. Cite both the name and the assertion, and give the structural fix — import the bound and assert against it, or narrow the name to what is proven (`tdd/tests.md` § *A test name is a claim the assertion must be able to falsify*). Do not recommend "check the name matches the body"; that is the attentional version, and it is what already failed. **Severity split:** **Suggestion** by default. **Concern** when the named relationship is to a value the same diff introduces, or to one living in a dependency — the name is then load-bearing documentation of a constraint nothing enforces, and it is the sentence that gets quoted into docs. **Review-cadence note.** Added from one triggering incident (an anonymized downstream Python/TypeScript service, 2026-08-07 — an LLM-client swap whose token-budget test named a ceiling and pinned a literal; the fifth instance of one shape in that repo, and the first authored by an agent that had read the repo's own `docs/solutions/` entry about it two hours earlier) plus principle grounds (Ammann & Offutt: the name is the test requirement, the assertion is the oracle, and coverage and oracle are orthogonal). Note this detector was already reliable *when noticed* — it caught all five instances attentionally — so the bullet exists to make it mechanical, not because it was failing. Same falsification rule as its siblings: if after a reasonable sample of PRs it fires <10% of the time on diffs that had no other reported issue, remove it rather than leave it as ceremony.
