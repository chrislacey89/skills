...
- **Recommended step first.** Pre-surface the `Comes next by default:` step as the first/default option — Nudge's "defaults are the master lever." Label it so the recommendation is obvious (e.g. "→ `/pre-merge` (recommended)").
