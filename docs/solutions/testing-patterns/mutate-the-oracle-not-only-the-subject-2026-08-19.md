...
**Amended 2026-08-24 — component coverage is not direction coverage.** The rule above enumerates *parts* (subject, fixture, oracle), and the second instance below had no fixture and no oracle to enumerate. Add the orthogonal half:
...
And its mirror, for the battery rather than the assertion — the question the second instance below needed and nobody asked:
...
**Second instance (2026-08-24).** Recorded in-place rather than as a new entry: `testing-patterns` already holds four entries whose `problem_type` names a guard that under-covers what it claims, and `/compound` Phase 4 forbids a further prose-only entry on a recurring pattern. The mechanism this instance needs is filed as **#277** and is *not yet built* — the reachability floor belongs in a suite PR #276 does not otherwise touch, so it was filed rather than absorbed. That is the honest state: this instance ships a mechanism-in-waiting, not a mechanism.
