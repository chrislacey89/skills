...
- **Sibling docs:**
  ...
  - `disclosed-weakness-needs-an-outside-reader-2026-08-16.md` — **the child**, and a second instance of this entry's suppression effect (§ Root Cause, second half). It isolates the disclosure mechanism from the composition mechanism and supplies the half stated only from the author's side here: what a disclosed weakness does *for an outside reader*. It also carries evidence on the open question in § Shelf Life below.
...
**First evidence on that divergence, and it runs against § Solution.** On PR #241 (2026-08-16) the `## Review Notes` were forwarded verbatim to the blind reviewer under #222's shipped behavior, including a `known-weak spots` line conceding that one clause of the change's own new rule was "satisfiable by assertion rather than by checking." The disclosure did not mute the reviewer. It **aimed** it: the sub-agent quoted the sentence back, went to that clause first, and returned two violations of it as its top Concern. One instance does not settle a question this entry argues from one instance in the other direction — but it is the first data point, and it suggests the mute is an author-side effect that does not survive the handoff to a reader with no memory of filing it. See `disclosed-weakness-needs-an-outside-reader-2026-08-16.md`, which is built on that instance and states the asymmetry directly.
