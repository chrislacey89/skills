...
- **Sibling docs:**
  - `sweep-commits-reintroduce-their-own-defect-class-2026-08-18.md` — the mechanism from this entry, observed doing its job four times running. Every instance in that entry was caught by a fresh-context sub-agent and none by the session that wrote the sweep, which is this entry's claim applied to corrections rather than to features.
