...
components: [pre-merge, execute, compound, docs, skill-references]
...
- **Level:** Pattern (three instances, one branch, one author, one week). Not yet Structure — see Rule Scope.
...
- **Inverts or does not apply when:**
  ...
  - The sweep is over a **generated** surface. The ten `<skill>/references/` mirrors in this PR were never at risk; `sync-skill-references.sh --check` makes a partial sweep impossible by construction.
...
**Alternatives considered:** A `/pre-merge` checklist item alone (rejected — advisory, and the advisory version is what missed three times); generating the restatements from a single source (rejected for now — the four skills that cannot read the core genuinely need the text inline, and a generator is a heavier mechanism than a grep-and-compare suite).
**Revisable:** Yes. If the restatement count grows past a handful of docs, generation beats pinning and this decision should flip.

## Related

- PR #246, issue #245 — the branch this pattern was observed on
- `scripts/test-options-comparison-contract.sh` — the mechanism
