...
**Unlike rows 1–3 of the earlier tables, the third row's defect was still open when this observation was first written.** It was corrected in `5a87cd7`, which splits the two-item tail onto separate, explicitly named axes instead of one shared one: the working files stay banned because they can move, and the fixer's report is barred on a different ground — independence from the fixer's own account is the breaker's reason to exist, not mutability.
...
- **Applies when** the first holds, together with the unchecked relation the last bullet names; the second and third are typical but not necessary. Observation 3 is why bullet 2 was dropped from the necessary set: its search-scoped instances (a color parser blind to a value shape, a claim parser blind to a rewording) fit bullet 2, but its two inert-guard instances — an `exit` inside `$( … )`, a `( cd … ) || fatal` fixture — are not sweeps enumerated by any query, and the structure held for them anyway. Observation 4 (2026-09-04) is why bullet 3 was dropped too: three independently authored corrections, sharing no context and so no falsified model to author from, reproduced the structure anyway — see Root Cause.
  ...
  - The correction is often written in **the same medium** as the defect (prose fixing prose, a path fixing a path, a check fixing a check). This bullet stays in the list because the same-medium case is common, not because it is required: observation 4 (2026-09-04) demoted its original ground — "the author's just-falsified model is the one authoring the fix" — from a necessary condition to an aggravator that raises the rate, after three fresh, unrelated authors reproduced the structure with no shared model between them.
...
Stable, and now evidenced across four branches and two media (prose, executable checks) — the promotion condition below is met, so it is no longer pending. Supersede if the repo adopts generation for cross-file restatements (which would make the enumeration structural rather than search-scoped), or if a later sample shows sweeps landing clean without a pinning test. (A clause here reading "this is one branch and one author, which is why Rule Scope calls it Pattern and not Structure" was stale from the 2026-08-26 promotion and survived the observation-3 rewrite that contradicted it two clauses earlier — this entry, committing this entry's defect class, for the third time — and the fourth observation's first draft did it three more times, corrected in `0a0e0e0`. Caught by the review sub-agent, not by the sweep.) The 2026-08-26 note asked for two or three more branches before the claim could be about how corrections are authored generally rather than about this repo's prose surfaces. Observation 3 supplied the third branch and, more usefully, the second medium: the structure held where there was no prose, no second site, and nothing to sync. Observation 4 (2026-09-04) supplies the fourth branch and removes the last author-shaped reading: three corrections, three independent fresh contexts, same structure. Treat the claim as general.

What would still falsify it: a branch landing clean is no longer a sufficient falsifier by itself — observation 4 shows single-context authorship was never the necessary condition, so a clean branch authored by one context proves nothing new. The falsifier is now a *seam* — the paragraph, function, or contract under correction — that survives several independent corrective rounds clean, under the same review intensity. A mechanism class that reliably catches these at authoring time remains separately relevant: detector C and the round-3 self-tests are the first evidence that is possible.
