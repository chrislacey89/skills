...
The *general* rule outlives it and is now one entry in a chain saying one thing five ways: **a limitation, a source gap, a stamped interval, a verification, or a detection that routes nowhere actionable is a footnote, not a mechanism.** The fifth is `fix-commit-blindness-2026-08-15.md`, where `/pre-merge`'s review-currency stamp correctly detects that the head has moved past the reviewed SHA and reports it only to the merge step. Five instances is well past the point where the siblings should be consolidated into one entry; that consolidation is the real successor to all five, and this entry should be folded into it rather than deleted.
