...
**What is different this time: the mechanism shipped in the same change.** `scripts/test-compound-clustering-single-source.sh` pins the retired phrasings out of every tracked file and asserts `compound/SKILL.md` is the rule's only declaring site, with `SYSTEM-OVERVIEW.md` permitted to *refer* by name but never to carry a threshold. It was mutation-tested against both real incidents — reintroducing the stale sentence in `SYSTEM-OVERVIEW.md`, and planting it only in the `vite-project` fixture the sweep missed — plus three synthetic mutations, all five caught, per `../testing-patterns/mutate-the-oracle-not-only-the-subject-2026-08-19.md`.

**The finding is not "the entry worked."** It did not fire at authoring time, for an author who had this file open and had grepped this directory. It fired because an independent review pass in a fresh context found the stale copy, and a second pass found the copy the first fix missed. That is now the *second* recorded instance (with the fifth) of this entry failing to fire for a reader who had read it — which is evidence that the consolidation's value is not a better retelling but a **trigger a machine can evaluate**. #208 is now overdue by two instances, and the honest read is that prose consolidation would be the seventh instance rather than the cure.
