...
**Updated 2026-08-20 — the sixth instance landed, exactly as the sentence above predicted, and it is folded in here for the same reason the fifth was.** PR #259 (issue #257) changed `/compound` Phase 4's clustering rule and left `SYSTEM-OVERVIEW.md:207` restating the old one, which `skill-references.manifest` bundles into four more copies — one rule, six hand-maintained sites, `sync-skill-references.sh --check` green throughout because the copies matched their source and the source was wrong. That is this entry's Problem statement with the nouns changed.

**It then fired twice more inside its own correction, which is the part worth reading.** The sweep commit missed a sixth copy (`vite-project/src/data/workflow-data.js:108`, tracked, referenced from nowhere). And the changelog bullet written to *narrate this very lesson* closed with "with one copy left there is nothing to pin" — false when written, and the premise it used to decline shipping a mechanism. See `sweep-commits-reintroduce-their-own-defect-class-2026-08-18.md`; the two entries are the same event seen from two angles.

**And then the mechanism written to close it became the seventh instance.** The first draft of `scripts/test-compound-clustering-single-source.sh` was **red on a clean tree the moment it was staged**, and would have blocked every PR from the moment it landed. Its `retired[]` array holds the literal phrases it forbids, and it scanned `git ls-files` — which lists tracked files and not untracked ones. So every mutation in its battery was run *before* `git add`, passed, and the suite was never run once after. The author had already spotted the self-reference problem for a neighboring check and written a comment exempting `scripts/` from it; they did not carry the thought one check to the left.

This is the sharpest form the family takes: **an artifact whose entire purpose is to stop a claim outrunning its mechanism, shipping with its own claim outrunning its own mechanism.** Compare `sweep-commits-reintroduce-their-own-defect-class-2026-08-18.md`, whose last row is the same shape one turn earlier — the contract test written to prevent a defect class containing the defect class.

**Two more defects rode in with it**, both of the same kind — an assertion narrower than the sentence describing it. The `SYSTEM-OVERVIEW` check gated on a *literal phrase*, so a threshold restated in new words — which is what a restatement is — took a vacuous pass; the check would not have caught the very incident in its own header comment. And a `grep -v '^scripts/'` filter, written to let the suite name what it forbids, let *any* script carry the rule and a threshold invisibly.

**What the rewrite changed, and the discipline that found it.** The suite now excludes only itself, asserts its file list is non-empty before trusting any absent-result, distinguishes *declaring* the rule (a bold heading, allowed in one file) from *referring* to it by name (allowed anywhere, in any case), checks all **five** `SYSTEM-OVERVIEW` copies rather than the canonical alone, and matches thresholds by **shape** rather than by remembered wording. Eleven mutations now: five oracle, four subject, two false-red regressions.

The battery that missed the defect had **zero oracle mutations** — and `../testing-patterns/mutate-the-oracle-not-only-the-subject-2026-08-19.md:120` requires *"at least one mutation of the fixture and at least one of the oracle, not only of the artifact under test."* The first battery cited that entry while satisfying a different axis it invented (real incidents vs. synthetic ones). The oracle mutation on the file-list pathspec is exactly what exposes the defect, in one step.

**The finding is not "the entry worked."** It did not fire at authoring time, for an author who had this file open and had grepped this directory — twice, now, in a single PR. It fired because three independent review passes in fresh contexts found, in turn: the stale copy, the copy the first fix missed, and the mechanism that could never have gone green. That is now **three recorded instances** (with the fifth) of this entry failing to fire for a reader who had read it, against zero instances of it firing at authoring time.

That asymmetry is the datum. A better retelling of the diagnosis would not have changed any of the three outcomes, because every author had already read the diagnosis. What changed the outcome each time was an **external checker in a context that did not hold the author's intent**. So the consolidation #208 is holding open is worth writing only if it is shaped as a trigger a machine or an independent reader can evaluate — and **#208 is now overdue by three**. The honest read is that a prose consolidation would be the eighth instance rather than the cure.
