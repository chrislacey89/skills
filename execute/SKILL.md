...
- **AFK Ralph iteration** (Step 5 structurally cannot run — there is no user to ask) → enter **`/pre-merge` loop-mode** rather than exiting to a manual invocation. This is the change that moves the AFK boundary from "code exists" to "code has been reviewed and every finding has an owner." Loop-mode escalates rather than deciding architecture, contracts, or dependencies for itself, so what the operator inherits in the morning is a reviewed branch plus a short list of genuine judgment calls.
