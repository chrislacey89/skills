...
- **Each option must be an unambiguous choice on its own.** Every rule above shapes the menu as a *set* — its ordering, its size, its escape hatch. This one shapes the individual option. Krug's Second Law — *"it doesn't matter how many times I have to click, as long as each click is a mindless, unambiguous choice"* — locates the cost of a choice in thought required × uncertainty about correctness, not in count or position. A menu can satisfy all four rules above and still offer options nobody can choose between. Check each option against Krug's three sources of ambiguity:
