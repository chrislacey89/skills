...
**Output:** A GitHub PR with structured body, stamped at the end of the review with a `## Review Currency` block naming the head SHA the review actually covered. Advisory findings in terminal.
...
| `/pre-merge` | Verified implementation ready to review (author-mode), or an existing PR number to review (reviewer-mode via `--pr <n>`) | PR with lineage, stamped with the reviewed head SHA, plus architectural review readout (author-mode), or draft PR comment text shaped by `references/comment-craft.md` (reviewer-mode) | `/compound` in-PR when a lesson emerged, then `/closeout` to merge and tear down |
