...
**What it does:** Creates a GitHub PR with full lineage (links PRD issue, lists closed/open slice issues) and runs an architectural review of the full diff against project principles. The review dimensions live in `pre-merge/review-checklist.md`, and that file's numbered headings are the roster — neither this document nor `pre-merge/SKILL.md` restates the list or its size, because a hand-copied restatement of it dropped a merge-blocking dimension on the day that dimension was added and stayed wrong for months while the review reported full coverage. Each dimension declares its own owner in the checklist. Most are handed to a fresh review sub-agent that sees the diff and nothing of the authoring session. The rest are marked controller-owned and run in Phase 4 instead, because their procedures need a cross-slice view — the PRD, every slice issue, and the set of merged slices held together — that only the controller assembles; coverage-matrix reconciliation, which checks the PRD's Must-commitments against the slices that actually merged, is one of them. Several dimensions gate themselves off when the diff gives them nothing to check: boundary maps and coverage matrix need a PRD with slice issues, runtime initialization needs schema/config/CLI/deploy-runtime changes, and fix completeness needs a bug fix (it catches workarounds disguised as corrections, missing regression tests, and lone instance fixes). Findings are advisory — presented in the terminal as Observations, Suggestions, or Concerns. When a PRD with slice issues was provided, also notes scope drift: work that appeared in the diff but wasn't in any slice's Boundary Map, and declared Produces that weren't implemented.
