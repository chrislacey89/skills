...
| `/visual-recap` | Finished diff, PR, or branch to comprehend, plus the base branch to diff against | Transient self-contained interactive HTML artifact in the selected mode — scrolling recap (file-tree + change flags, annotated split diffs with line-anchored callouts, schema/API contract cards, UI wireframes, before/after columns) or walkthrough deck (premise → changes → mechanism → aftermath, one idea per screen) — with CSS-spine diagrams for trivial flows and Mermaid-via-CDN for multi-stage or behavioral graphs, plus the reviewer's feedback (copied back where the artifact is handed off), with durable items promoted to GitHub PR review comments | `/closeout` to merge and tear down, then `/compound` only when a durable lesson emerged |
