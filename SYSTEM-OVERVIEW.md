...
| `/fix-findings` | The `/pre-merge` finding numbers a human chose, plus the findings themselves (terminal output, or the PR ledger loop-mode wrote) | One commit per accepted finding, each authored by a fresh sub-agent that did not write the code; a per-finding breaker verdict (`killed` / `survived` / `not-run` / `not-applicable`) with the command behind it; `refuted` reports for findings the tree did not confirm. No stamp, no PR edit, no merge | a re-run of `/pre-merge` in author-mode, which re-runs the dimensions over the whole branch diff from the base ref — these fix commits included, since no mode takes a range or scopes itself to what is past the stamp — and re-stamps; then `/compound` if a lesson emerged, then `/closeout` |
