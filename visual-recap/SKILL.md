...
Those first three are one decision at three stages, and a live plan surface usually shows more than one stage at once: a question that has no options yet, a fork rendered as a matrix, a choice already made and rendered as a card. **When a card records the outcome of a matrix on the same surface, carry the winning option's slug into the card's id** — `opt-refuse-to-assemble` becomes `dc-refuse-to-assemble` (§13). Numbering the decisions instead is the failure: insert one, regenerate, and every answer already pasted back attaches to a different decision with nothing reporting it.
